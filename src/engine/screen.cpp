    ...
    private:
        ...
        bool allocate( fheroes2::ResolutionInfo & resolutionInfo, bool isFullScreen ) override
        {
            ...
            if ( _bottomWindow == nullptr ) {
                ...
                if ( _bottomRenderer == nullptr ) {
                    ...
                    _bottomTexture = SDL_CreateTexture( _bottomRenderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_STREAMING, resolutionInfo.gameWidth, resolutionInfo.gameHeight );
                    if ( _bottomTexture == nullptr ) {
        ...
        void render( const fheroes2::Display & display, const fheroes2::Rect & roi ) override
        {
            ...
            if ( _bottomRenderer != nullptr && _bottomTexture != nullptr ) {
                ...
                if ( bottomReturnCode < 0 ) {
                    ...
                    const float scale = std::min( static_cast<float>( N3DS_BOTTOM_SCREEN_WIDTH ) / display.width(),
                                                 static_cast<float>( N3DS_SCREEN_HEIGHT ) / display.height() );
        ...
        void _calculateScreenScaling( const int32_t width_, const int32_t height_, const bool isFullScreen )
        {
            ...
            if ( !isFullScreen ) {
                ...
                const float scale = std::min( static_cast<float>( N3DS_TOP_SCREEN_WIDTH ) / width_,
                                             static_cast<float>( N3DS_SCREEN_HEIGHT ) / height_ );
